...
const vec3 sunlight_color = vec3(1.051, 0.985, 0.94); // Color of sunlight in space, obtained from AM0 solar irradiance spectrum from https://www.nrel.gov/grid/solar-resource/spectra-astm-e490.html using the CIE (2006) 2-deg LMS cone fundamentals

const vec3 sun_irradiance = sunlight_color;
const vec3 moon_irradiance = sunlight_color * 0.00025;

const float sun_angular_radius = 0.004;
const float moon_angular_radius = 0.008;

const ivec2 transmittance_res = ivec2(
    /* mu */
    256,
    /* r */
    64
);
const ivec3 scattering_res = ivec3(
    /* nu */
    16,
    /* mu */
    64,
...
const mat2x3 air_scattering_coefficients = mat2x3(
    air_rayleigh_coefficient,
    air_mie_albedo * air_mie_coefficient
);
...
vec3 atmosphere_scattering(
    ...
    uv.x *= 0.5;
    scattering = texture(lut, uv).rgb;

    // Single mie scattering
    uv.x += 0.5;
    scattering += texture(lut, uv).rgb * mie_phase;
...
vec3 atmosphere_scattering(
    ...
    return atmosphere_scattering(lut, nu, mu, mu_s, use_klein_nishina_phase);
}
...
vec3 atmosphere_scattering(
    ...
    vec3 scattering_sc = texture(lut, uv_sc).rgb;
    vec3 scattering_sm = texture(lut, uv_sm).rgb;
    vec3 scattering_mc = texture(lut, uv_mc).rgb;
    vec3 scattering_mm = texture(lut, uv_mm).rgb;
...
vec3 atmosphere_transmittance(float mu, float r) {
    if (intersect_sphere(mu, max(r, planet_radius + 10.0), planet_radius).x >= 0.0)
        return vec3(0.0);
