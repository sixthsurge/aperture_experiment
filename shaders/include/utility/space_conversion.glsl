...
vec3 screen_to_view_space(mat4 projection_matrix_inverse, vec3 screen_pos, bool handle_jitter) {
	vec3 position_ndc = 2.0 * screen_pos - 1.0;
	...
	if (handle_jitter) position_ndc.xy -= jitter_offset;
#endif

	return project_and_divide(projection_matrix_inverse, position_ndc);
}

vec3 view_to_screen_space(mat4 projection_matrix, vec3 view_pos, bool handle_jitter) {
	vec3 position_ndc = project_and_divide(projection_matrix, view_pos);
